...
class kernel_smoother:
    ...
    def local_general(self, kernel='gaussian', D=1):
        """
        Parameters
        ----------
            kernel: str (optional)
        ...
        for i in range(self.y_star.shape[0]):
            ...
            # Instantiates X matrix 
            # X = np.transpose(np.array([np.ones(self.x.shape[0]), (self.x - self.x_star[i])]))
            X = np.transpose(np.ones((self.x.shape[0], D+1)))
    ...
    def MSE_optimization(self, y_test, smoother='local_constant'):
        """
        Parameters
        ----------
            y_test: float
