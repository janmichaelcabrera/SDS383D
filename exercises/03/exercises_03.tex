...
plt.close()
        ...
        Note here that the code was re-run using the Matern (5,2) covariance function.
            ...
            We reintroduce this back into the exponential term. The term $\int \text{exp}\left[-\frac{1}{2} (f-h)^T A (f-h) \right] df$ integrates to a constant not dependent on $y$. 
            ...
            Note that with $ \left(\frac{1}{\sigma^2} I\right)^{-1} = \sigma^2 I $ the marginal for $p(y)$ becomes,
